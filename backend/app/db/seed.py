...
async def seed_roles_permissions(db: AsyncSession) -> None:
    ...
    # Assign permissions to roles via raw insert to avoid lazy loading
    for role_name, perm_names in ROLE_PERMISSIONS.items():
        ...
        for perm_name in perm_names:
            ...
            if not exists.fetchone():
                await db.execute(
                    sql_insert(role_permissions_table).values(role_id=role.id, permission_id=perm.id)
                )
...
async def seed_super_admin(db: AsyncSession) -> None:
    ...
    if admin is None:
        ...
        # Attach Super Admin role via raw insert
        role_result = await db.execute(select(Role).where(Role.name == "Super Admin"))
        sa_role = role_result.scalar_one_or_none()
        if sa_role:
            await db.execute(
                sql_insert(user_roles_table).values(user_id=admin.id, role_id=sa_role.id)
            )
